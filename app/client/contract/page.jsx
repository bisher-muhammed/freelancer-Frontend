...
export default function ClientContractPage() {
  ...
  const fetchContracts = async () => {
    try {
      setLoading(true);
      const response = await apiPrivate.get('/client/contracts/');
  ...
  // ESCROW STATUS BADGE
  // ============================
  const getEscrowStatusBadge = (escrowPayment) => {
    if (!escrowPayment) {
      return (
    ...
    const statusConfig = {
      escrowed: { bg: 'bg-green-100 text-green-700', label: 'Escrowed' },
      released: { bg: 'bg-blue-100 text-blue-700', label: 'Released' },
      refunded: { bg: 'bg-orange-100 text-orange-700', label: 'Refunded' },
      pending: { bg: 'bg-yellow-100 text-yellow-700', label: 'Pending' }
    };

    const config = statusConfig[escrowPayment.status] || {
      bg: 'bg-gray-100 text-gray-600',
      label: escrowPayment.status
    };
  ...
  const navigateToDetails = (contractId) => {
    router.push(`/client/contract/${contractId}`);
  };
  ...
  const getStats = () => {
    ...
    // ✅ Total Spent = Sum of Escrow Budgets (from offer.total_budget or escrow_payment.amount)
    stats.totalSpent = contracts.reduce((sum, c) => {
      const escrowAmount = c.escrow_payment?.amount || c.offer?.total_budget || 0;
      return sum + Number(escrowAmount);
    }, 0);

    // Count contracts with active escrow
    stats.escrowedCount = contracts.filter(c => c.escrow_payment?.status === 'escrowed').length;
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 mb-6">
              You haven't created any contracts yet. Start by hiring a freelancer!
            </p>
            <button
              onClick={() => router.push('/find-freelancers')}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Escrow-protected contracts with freelancers
              </p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1">
                <div className="relative">
                  <input
                    type="text"
                    placeholder="Search contracts by project, client, or scope..."
                    value={searchTerm}
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0">
                {['all', 'active', 'completed', 'terminated', 'disputed', 'pending'].map(
                  (status) => (
                    <button
                      key={status}
          ...
          {filteredContracts.map((contract) => {
            ...
            return (
              <div
                ...
                className="bg-white rounded-xl border border-gray-200 hover:border-indigo-300 hover:shadow-lg transition-all duration-200 cursor-pointer group"
              >
                <div className="p-6">
                  {/* Header */}
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        ...
                        {getEscrowStatusBadge(contract.escrow_payment)}
                  ...
                  {escrowData.escrowedAt && (
                    <div className="mb-4 p-3 bg-green-50 rounded-lg">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-xs text-gray-600">Funds Escrowed</p>
                  ...
                  {/* Action Button */}
                  <button
                    className="w-full py-3 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-colors font-medium flex items-center justify-center gap-2"
                  >
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-xl border p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-900">
                  {formatMoney(stats.total > 0 ? stats.totalSpent / stats.total : 0)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-600">Platform Fees Est.</span>
                <span className="font-medium text-green-600">
                  {formatMoney(stats.totalSpent * 0.1)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border p-6">
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={() => router.push('/client/browse-freelancers')}
