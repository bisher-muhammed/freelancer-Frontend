...
export default function FreelancerChatRoomPage() {
    ...
    <div className="flex h-screen bg-[#F0F2F5] dark:bg-gray-900 font-sans text-gray-900">
      ...
      <div className={`flex-1 flex flex-col min-w-0 bg-white dark:bg-gray-800 shadow-xl transition-all duration-300 ${
        ...
        <header className="h-16 flex items-center justify-between px-4 border-b dark:border-gray-700 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md z-10">
          <div className="flex items-center gap-3">
            ...
            <div className="relative cursor-pointer group">
              <div className="w-10 h-10 bg-gradient-to-tr from-purple-500 to-pink-600 rounded-full flex items-center justify-center text-white font-bold">
            ...
            <div>
              <h1 className="font-semibold text-sm md:text-base leading-none">{chatTitle || "Client Chat"}</h1>
        ...
        <footer className="bg-white dark:bg-gray-800 p-3 border-t dark:border-gray-700">
          {replyTo && (
            <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-700 p-2 px-4 rounded-t-lg border-l-4 border-purple-500 mb-2">
              <div className="text-xs">
                <p className="font-bold text-purple-600">Replying to {replyTo.sender_name || 'User'}</p>
          ...
          <div className="flex items-end gap-2 max-w-6xl mx-auto">
            <div className="flex items-center bg-gray-100 dark:bg-gray-700 rounded-2xl flex-1 px-3 min-h-[48px]">
              <button className="p-2 text-gray-500 hover:text-purple-500"><Smile className="w-6 h-6" /></button>
              <button onClick={() => fileInputRef.current?.click()} className="p-2 text-gray-500 hover:text-purple-500">
            ...
            <button 
              ...
              className="w-12 h-12 flex items-center justify-center bg-purple-600 hover:bg-purple-700 disabled:bg-gray-400 text-white rounded-full transition-all shadow-md active:scale-90"
            >
  ...
  if (msg.type === 'date') {
    return (
      <div className="flex justify-center my-4">
        <span className="bg-purple-100 dark:bg-gray-800 text-purple-700 dark:text-gray-400 text-[11px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">
    ...
    <div className={`flex ${isMe ? 'justify-end' : 'justify-start'} ${msg.isContinuation ? 'mt-0.5' : 'mt-4'}`}>
      <div className={`group relative max-w-[85%] md:max-w-[70%] flex gap-2 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className={`relative px-3 py-2 rounded-2xl shadow-sm text-sm ${
          ...
          {!isMe && !msg.isContinuation && (
            <p className="text-[11px] font-bold text-purple-500 mb-1 leading-none">{msg.sender_name || 'Client'}</p>
          )}
          ...
          <button 
            onClick={() => onReply(msg)}
            className={`absolute top-0 ${isMe ? '-left-8' : '-right-8'} p-1.5 opacity-0 group-hover:opacity-100 transition-opacity bg-white dark:bg-gray-700 rounded-full shadow-sm hover:text-purple-500`}
          >
