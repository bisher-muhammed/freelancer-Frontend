...
export default function FreelancerChatList() {
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      try {
        const res = await apiPrivate.get("chat-rooms/freelancer/");
        const data = res.data;
  ...
  const filteredRooms = useMemo(() => {
    return rooms.filter((room) => {
      const title = room.project?.title?.toLowerCase() || "";
      const client = room.client?.name?.toLowerCase() || "";
        ...
        {loading ? (
                    ...
                    <div className="flex items-center justify-between mb-0.5">
                      <div className="flex items-center gap-2">
                        <h3 className="font-semibold text-gray-900 truncate group-hover:text-blue-600 transition-colors">
                          {room.project?.title || "Untitled Project"}
                        </h3>
                        {room.unread_count > 0 && (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-gray-500 text-sm mt-1">
              {searchQuery ? "Try adjusting your search terms." : "Start a conversation by getting hired for a project."}
            </p>
