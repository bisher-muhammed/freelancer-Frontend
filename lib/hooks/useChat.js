...
export function useChat(chatId) {
  ...
  const socketUrl =
  `${protocol}://api.freelancerhub.online/ws/chat/${chatId}/?token=${accessToken}`;
        ...
        if (e.code !== 1000 && e.code !== 1001) {
          setTimeout(() => {
            if (chatId && accessToken) {
              const reconnectUrl = `${protocol}://api.freelancerhub.online/ws/chat/${chatId}/?token=${accessToken}`;
